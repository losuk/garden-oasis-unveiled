...
export function Header() {
  ...
  return (
    <>
        ...
        <div className="mx-auto flex max-w-[1600px] items-center justify-between px-4 md:px-6 lg:px-12 h-[64px] lg:h-[72px]">
          <Link to="/" className="flex items-center">
            <img
              ...
              alt="The Emin Pasha"
              className={`h-12 md:h-14 transition-all duration-250 ${!solid ? "brightness-0 invert" : ""}`}
            />
          </Link>
          ...
          >
            {primaryNav.map((item) => (
              <div key={item.to} className="relative group">
                <Link
                  to={item.to as any}
                  className="flex items-center gap-1 opacity-90 hover:opacity-100 hover:text-accent transition-colors"
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-[99999] bg-[var(--color-footer)] flex flex-col">
